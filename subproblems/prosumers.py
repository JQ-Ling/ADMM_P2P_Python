...
def subproblem_prosumer(Param, Pout_aux, user_id, lamda_u, rho_u):
    """
    ...
    # === Net Load Equations ===
    for t in range(h):
        m.addConstr(nload[t] == P_load[t] + P_c[t] - P_d[t])
        m.addConstr(Pg_buy[t] + P_buy[t] == nload[t] * B_load[t])
        m.addConstr(Pg_sell[t] + P_sell[t] == nload[t] * (B_load[t] - 1))
