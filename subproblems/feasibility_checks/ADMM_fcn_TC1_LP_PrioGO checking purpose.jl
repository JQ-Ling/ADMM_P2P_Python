...
function Subproblem_Prosumer(Param::Dict{}, Pout_aux, num_user, lamda_u, num_iteration; check=false) # solve subproblem for each user
    ...
    ε = 1e-6
    @constraint(Prosumer_model, P_out .>= Pout_aux[:, num_user] .- ε)
    @constraint(Prosumer_model, P_out .<= Pout_aux[:, num_user] .+ ε)
    @objective(Prosumer_model, Min, 0)

    optimize!(Prosumer_model) # calculate the problem
    println("User $num_user: ", termination_status(Prosumer_model))

    return termination_status(Prosumer_model)
end


# subproblem for grid operator
function Subproblem_Grid_Operator(Param::Dict{}, P_out, lamda_u, iteration_num; check=false) # solve subproblem for each user
    hour = Param[:hour]
    ptdf = Param[:ptdf]
    ...
    ε = 1e-6
    @constraint(Grid_model, P_out_aux .>= P_out .- ε)
    @constraint(Grid_model, P_out_aux .<= P_out .+ ε)
    @objective(Grid_model, Min, 0)


    optimize!(Grid_model) # calculate the problem
    println("Grid Operator: ", termination_status(Grid_model))

    return termination_status(Grid_model)
end
