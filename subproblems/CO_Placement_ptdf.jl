...
function evaluate_fitness(ces_sizes::Vector{Float64}, ces_locs::Vector{Float64})
    ...
    # 4. PTDF Constraints (Network Physics)
    @constraint(Central_Model, P_inj .== -(net_load_kW * ptdf'))
    @constraint(Central_Model, [i in 1:hour, j in 1:num_branch], -DATA[:branch_limit][j] .<= P_inj[i, j] .<= DATA[:branch_limit][j])
